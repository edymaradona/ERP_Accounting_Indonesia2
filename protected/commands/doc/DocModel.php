...
class DocModel
{
    ...
    protected function processDescription($text)
    {
        if (($text = trim($text)) === '')
            return '';
        $text = preg_replace_callback('/\{@include\s+([^\s\}]+)\s*\}/s', [$this, 'processInclude'], $text);
        $text = preg_replace('/^(\r| |\t)*$/m', "<br/><br/>", $text);
        $text = preg_replace_callback('/<pre>(.*?)<\/pre>/is', [$this, 'processCode'], $text);
        $text = preg_replace_callback('/\{@link\s+([^\s\}]+)(.*?)\}/s', [$this, 'processLink'], $text);
        return $text;
    }
