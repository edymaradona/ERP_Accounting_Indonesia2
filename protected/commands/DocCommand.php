...
EOD;
    ...
    public function render($view, $data = null, $return = false, $layout = 'main')
    {
        ...
        return $this->renderFile($layoutFile, ['content' => $content], $return);
    }
    ...
    public function renderSourceLink($sourcePath, $line = null, $sourcePathType = null)
    {
        ...
        if ($url === null)
            ...
            if ($line === null) {
                return CHtml::link($text, $url, ['class' => 'sourceLink']);
            } else {
                return CHtml::link($text . '#' . $anchorLinePreffix . $line, $url . '#' . $anchorLinePreffix . $line, ['class' => 'sourceLink']);
            }
        }
    }
    ...
    protected function buildOfflinePages($docPath, $themePath)
    {
        ...
        $content = preg_replace_callback(self::URL_PATTERN, [$this, 'fixOfflineLink'], $content);
        file_put_contents($docPath . '/index.html', $content);
        foreach ($this->classes as $name => $class) {
            $this->currentClass = $name;
            $this->pageTitle = $name;
            $content = $this->render('class', ['class' => $class], true);
            $content = preg_replace_callback(self::URL_PATTERN, [$this, 'fixOfflineLink'], $content);
    ...
    public function renderTypeUrl($type, $sourcePathType = '')
    {
        if (isset($this->classes[$type]) && $type !== $this->currentClass)
            ...
            if ($sourcePathType === 'yii' && $this->withYiiLinks && !in_array($type, ['string', 'integer', 'int', 'boolean', 'bool', 'float', 'double', 'object', 'mixed', 'array', 'resource', 'void', 'null', 'callback', 'false', 'true', 'self'])) {
