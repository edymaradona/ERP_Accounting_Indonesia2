...
class Forum extends CActiveRecord
{
    ...
    public function rules()
    {
        ...
        return [
            ['title', 'required'],
            ['title', 'length', 'max' => 120],
            ['parent_id, description, is_locked', 'safe'],
            ['listorder', 'numerical', 'integerOnly' => true],
    ...
    public function relations()
    {
        ...
        return [
            'parent' => [self::BELONGS_TO, 'Forum', 'parent_id'],
            'subforums' => [self::HAS_MANY, 'Forum', 'parent_id', 'order' => 'listorder,title'],
            'subforumCount' => [self::STAT, 'Forum', 'parent_id'],
            ...
            'threadCount' => [self::STAT, 'Thread', 'forum_id'],
        ];
    }
    ...
    {
        return Yii::app()->createUrl('/forum/forum/view', ['id' => $this->id]);
    }
    ...
    public function getBreadcrumbs($currentlink = false)
    {
        ...
        while (null != $forum->parent_id) {
            ...
            $breadcrumbs[$forum->title] = ['/forum/forum/view', 'id' => $forum->id];
        }

        $breadcrumbs = array_merge(
            ['Forum' => ['/forum']], array_reverse($breadcrumbs)
        );

        if (!$this->isNewRecord) {
            $breadcrumbs = array_merge($breadcrumbs, $currentlink ? [CHtml::encode($this->title) => ['/forum/forum/view', 'id' => $this->id]] : [CHtml::encode($this->title)]
            );
        }
        return $breadcrumbs;
    }
    ...
    public function getPostCount()
    {
        return Yii::app()->db->createCommand()
            ...
            ->where('t.forum_id=:id', [':id' => $this->id])
            ->queryScalar();
    }
    ...
    public function renderLastpostCell()
    {
        ...
        $authorlink = CHtml::link(CHtml::encode($author->username), Yii::app()->createUrl('/sUser/viewAuthenticated', ['id' => $author->id]));
