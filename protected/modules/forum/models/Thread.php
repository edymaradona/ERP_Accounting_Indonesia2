...
class Thread extends CActiveRecord
{
    ...
    public function relations()
    {
        ...
        return [
            'forum' => [self::BELONGS_TO, 'Forum', 'forum_id'],
            'posts' => [self::HAS_MANY, 'Post', 'thread_id', 'order' => 'created'],
            'postCount' => [self::STAT, 'Post', 'thread_id'],
        ];
    }
    ...
    {
        return Yii::app()->createUrl('/forum/thread/view', ['id' => $this->id]);
    }
    ...
    {
        return array_merge(
            $this->forum->getBreadcrumbs(true), ($currentlink ? [CHtml::encode($this->subject) => ['/forum/thread/view', 'id' => $this->id]] : [CHtml::encode($this->subject)])
    ...
    public function getFirstPost()
    {
        $result = Post::model()->findByAttributes(
            ['thread_id' => $this->id], ['order' => 'created', 'limit' => 1]
        );
        return $result;
    }
    ...
    {
        $result = Post::model()->findByAttributes(
            ['thread_id' => $this->id], ['order' => 'created DESC', 'limit' => 1]
        );
        return $result;
    }
    ...
    public function renderSubjectCell()
    {
        ...
        $authorlink = CHtml::link(CHtml::encode($firstpost->author->username), Yii::app()->createUrl('sUser/viewAuthenticated', ['id' => $firstpost->author_id]));
