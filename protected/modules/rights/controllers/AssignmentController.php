...
class AssignmentController extends RController
{
    ...
    public function actionView()
    {
        ...
        $dataProvider = new RAssignmentDataProvider([
            'pagination' => [
                'pageSize' => 50,
            ],
        ]);
    ...
    public function actionUser()
    {
        ...
        if ($assignSelectOptions !== []) {
            ...
            if (isset($_POST['AssignmentForm']) === true) {
                ...
                if ($formModel->validate() === true) {
                    ...
                    Yii::app()->user->setFlash($this->module->flashSuccessKey, Rights::t('core', 'Permission :name assigned.', [':name' => $item->getNameText()])
                    );

                    $this->redirect(['assignment/user', 'id' => $model->getId()]);
                }
            }
        } // No items available
        else {
        ...
        $dataProvider = new RAuthItemDataProvider('assignments', [
            'userId' => $model->getId(),
        ]);
    ...
    public function actionRevoke()
    {
        ...
        if (Yii::app()->request->isPostRequest === true) {
            ...
            Yii::app()->user->setFlash($this->module->flashSuccessKey, Rights::t('core', 'Permission :name revoked.', [':name' => $item->getNameText()])
            );

            // if AJAX request, we should not redirect the browser
            if (isset($_POST['ajax']) === false)
                $this->redirect(['assignment/user', 'id' => $_GET['id']]);
        } else {
