...
class RAuthorizer extends CApplicationComponent
{
    ...
    {
        $exclude = $includeSuperuser === false ? [$this->superuserName] : [];
     ...
     */
    public function getAuthItems($types = null, $userId = null, CAuthItem $parent = null, $sort = true, $exclude = [])
    {
     ...
     */
    protected function excludeInvalidAuthItems($items, CAuthItem $parent = null, $exclude = [])
    {
        ...
        foreach ($items as $childName => $children) {
            if ($children !== []) {
                if (isset($children[$itemName])) {
                    ...
                    if (($p = $this->getAuthItemParentsRecursive($itemName, $children, $direct)) !== []) {
    ...
    private function getPermissionsRecursive(CAuthItem $item)
    {
        ...
        foreach ($item->getChildren() as $childName => $child) {
            ...
            if (($grandChildren = $this->getPermissionsRecursive($child)) !== [])
     ...
     */
    public function hasPermission($itemName, $parentName = null, $permissions = [])
    {
        if ($parentName !== null) {
...
// Get a list of all defined functions
        ...
        // Append a '(' to the functions to avoid confusion between e.g. [] and array_merge().
