...
class cAbsence1 extends fpdf
{
    ...
    function report($project_id, $departmen_id = 0)
    {
        ...
        foreach ($models as $model) {
            $this->SetFont('Arial', '', 8);
            $this->Cell($w[0], 4, peterFunc::indoFormat($_countert), 'LB', 0, 'R');
        ...
        $this->Cell($w[4], 5, '', 'TLB');
        $this->Cell($w[5], 5, '', 'TLB');
        $this->Cell($w[6], 5, peterFunc::indoFormat($_totalc), 'TLBR', 0, 'R');
        $this->Cell($w[7], 5, '', 'TLB');
        $this->Cell($w[8], 5, '', 'TLBR');
        $this->Ln(7);

        $w = [47.5, 47.5, 47.5, 47.5];
