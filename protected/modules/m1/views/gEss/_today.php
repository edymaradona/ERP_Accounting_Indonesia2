<div style="border: 1px #D5D5D5;border-bottom-style: solid;padding:3px 0;margin:10px 0;">
    <ul class="nav nav-list">
        <li class="nav-header"><i class="fa fa-bars fa-fw"></i><?php echo Yii::t('basic', ' Happening TODAY On Your Department') ?></li>
    </ul>
</div>
...
<div style="overflow: hidden;">
    <div style="min-height: 200px;max-height: 200px;overflow-y: auto;">
        ...
        <?php
        $dependency = new CDbCacheDependency('SELECT MAX(created_date) FROM g_leave');
        if (!Yii::app()->cache->get('essbirthday' . Yii::app()->user->id)) {
            $sql = '
		...
		WHERE (((select c.department_id from g_person_career c WHERE t.id=c.parent_id AND c.status_id IN 
		(' . implode(",", Yii::app()->getModule("m1")->PARAM_COMPANY_ARRAY) . ') ORDER BY c.start_date DESC LIMIT 1) = ' . $model->mDepartmentId() . ') 
		...
		(((select c.department_id from g_person_career c WHERE person.id=c.parent_id AND c.status_id IN 
		(' . implode(",", Yii::app()->getModule("m1")->PARAM_COMPANY_ARRAY) . ') ORDER BY c.start_date DESC LIMIT 1) = ' . $model->mDepartmentId() . ') 
		...
		';
            ...
            Yii::app()->cache->set('essbirthday' . Yii::app()->user->id, $rows, 3600, $dependency);
        } else
            $rows = Yii::app()->cache->get('essbirthday' . Yii::app()->user->id);
        ?>
