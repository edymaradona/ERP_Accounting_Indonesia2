...
class GBiPersonHoldingController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if (isset($_POST['field'])) {
            ...
            $fieldres['type'] = 'raw';
            $fieldres['value'] = '($data["c_pathfoto"] != null) ? CHtml::image(Yii::app()->request->baseUrlCdn . "/shareimages/hr/employee/thumb/" . $data["c_pathfoto"]) : ""';
            $html["width"] = "60px";
        ...
        if (isset($filter))
            Yii::app()->user->setFlash('success', 'CRITERIA : ' . $filter);

        $this->render('/gBiPerson/index2', [
            'model' => $model,
            'dataProvider' => $dataProvider,
            'field' => $fieldresult,
            'production' => $production,
