 ...
 */
class gParamExpenseDetail extends BaseModel
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            ['parent_id, status_id, group_id, sort, amount, created_date, created_by, updated_date, updated_by', 'numerical', 'integerOnly' => true],
            ['name', 'length', 'max' => 100],
    ...
    public function relations()
    {
        ...
        return [
            ...
            'childs' => [self::HAS_MANY, 'gParamExpenseDetail', 'parent_id', 'order' => 'childs.id ASC'],
            'status' => [self::BELONGS_TO, 'sParameter', ['status_id' => 'code'], 'condition' => 'type = "cOrganizationStatus"'],
            'link' => [self::HAS_ONE, 'gExpenseDetail', 'expense_id'],
        ];
    }
    ...
    public static function expenseDropDownInitial()
    {
        $_items = [];

        $criteria = new CDbCriteria;

        $criteria->compare('group_id', 1); //Inisial Expense Only
        ...
        foreach ($models as $model) {
            ...
            foreach ($modelchild as $mod)
                $_items[$model->name][$mod->id] = $mod->name;
        }
        return $_items;
    }
    ...
    public static function getExpenseLevel($expense,$level) {
        $criteria = new CDbCriteria;
        $criteria->compare('parent_id',$expense);
        $criteria->compare('level_id',$level);
        $model = self::model()->find($criteria);
