...
class profitLost2 extends fpdf
{
    ...
    function report($periode_date, $report_id)
    {
        ...
        foreach ($model1 as $mmm) {
            ...
            foreach ($mmm->account_list as $mm) {
                ...
                foreach ($model2->childs as $model) {
                    ...
                    $_modelLast = $model->balancesheet(['condition' => 'yearmonth_periode =' . $_periode_date_last]);
                    if (isset($_model->end_balance)) {
                        ...
                        $_balanceLast = peterFunc::indoFormat($_mmodelLast);
                    ...
                    if ($model->childs) {
                        foreach ($model->childs as $mod) {
                            ...
                            $_modLast = $mod->balancesheet(['condition' => 'yearmonth_periode =' . $_periode_date_last]);
                            if (isset($_mod->end_balance)) {
                                ...
                                $_balanceLast = peterFunc::indoFormat($_mmodLast);
                            ...
                            if ($mod->childs) {
                                ...
                                $this->SetX($this->x0);
                                $this->Cell($w[1], 4, peterFunc::indoFormat($_total), 'T', 0, 'R');
                                $this->Cell($w[1], 4, '', '');
                                $this->Cell($w[1], 4, peterFunc::indoFormat($_totalLast), 'T', 0, 'R');
                                $this->Ln(5);
                    ...
                    if ($model->childs && $model->childsCount >= 2) {
                        ...
                        $this->SetX($this->x0);
                        $this->Cell($w[1], 4, peterFunc::indoFormat($_subtotal), 'T', 0, 'R');
                        $this->Cell($w[1], 4, '', 0);
                        $this->Cell($w[1], 4, peterFunc::indoFormat($_subtotalLast), 'T', 0, 'R');
                        $this->Ln(5);
                    }
                ...
                $this->Cell($w[1], 4, peterFunc::indoFormat($_grandtotal), 0, 0, 'R');
                $this->Cell($w[1] + 20, 4, peterFunc::indoFormat($_grandtotalLast), 0, 0, 'R');
                $this->Cell($w[1], 4, '');
                $this->Ln(8);
            ...
            if ($mmm->id == 4) { //GROSS PROFIT
                ...
                $this->SetX($this->x0 + 20);
                $this->Cell($w[1], 4, peterFunc::indoFormat($_grossprofit), 'T', 0, 'R');
                $this->Cell($w[1], 4, '');
                $this->Cell($w[1], 4, peterFunc::indoFormat($_grossprofitLast), 'T', 0, 'R');
                $this->Ln(8);
            }
        }
        ...
        $this->SetX($this->x0 + 20);
        $this->Cell($w[1], 4, peterFunc::indoFormat($_netprofit), 'T', 0, 'R');
        $this->Cell($w[1] + 20, 4, peterFunc::indoFormat($_netprofitLast), 'T', 0, 'R');
        $this->Ln(8);
        ...
        foreach ($model1 as $mmm) {
            ...
            foreach ($mmm->account_list as $mm) {
                    ...
                    if ($model->childs) {
                        foreach ($model->childs as $mod) {
                            ...
                            if (isset($_mod->end_balance)) {
                                ...
                                $_balanceLast = peterFunc::indoFormat($_balanceRLast);
                ...
                $this->Cell($w[1], 4, peterFunc::indoFormat($_grandtotal), 0, 0, 'R');
                $this->Cell($w[1], 4, '');
                $this->Cell($w[1], 4, peterFunc::indoFormat($_grandtotalLast), 0, 0, 'R');
                $this->Ln(8);
            ...
            $this->SetX($this->x0 + 20);
            $this->Cell($w[1], 4, peterFunc::indoFormat($_netprofitFinal), 'T', 0, 'R');
            $this->Cell($w[1], 4, '');
            $this->Cell($w[1], 4, peterFunc::indoFormat($_netprofitFinalLast), 'T', 0, 'R');
            $this->Ln(8);
        }
    }
