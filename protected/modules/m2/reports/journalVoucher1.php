...
class journalVoucher1 extends fpdf
{
    ...
    function report($id)
    {
        ...
        foreach ($modelD as $mod) {
            ...
            $this->Cell($w[1], 5, peterFunc::indoFormat($mod->debit), 'LR', 0, 'R');
            $this->Cell($w[2], 5, peterFunc::indoFormat($mod->credit), 'R', 0, 'R');
        ...
        $this->Cell($w[1], 6, peterFunc::indoFormat($model->journalSum), 'TLR', 0, 'R');
        $this->Cell($w[2], 6, peterFunc::indoFormat($model->journalSum), 'TR', 0, 'R');
        $this->Cell($w[3], 6, '', 'TR');
        $this->Ln();
