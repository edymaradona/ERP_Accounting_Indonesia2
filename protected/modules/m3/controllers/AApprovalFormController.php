...
class AApprovalFormController extends Controller
{
    ...
    {
        aPorder::model()->updateByPk((int)$id, ['payment_state_id' => 3, 'payment_date' => new CDbExpression('NOW()')]);
        aPorderDetail::model()->updateAll(['detail_payment_id' => 3], [
    ...
    {
        aPorderDetail::model()->updateByPk((int)$id, ['detail_payment_id' => 3]);
    }
    ...
    public function actionUpdateApproved($id)
    {
        ...
        if ($model->approved_date == null) {
            ...
            #----------------------------------------
            #Budget Detail
            $modelbd = aBudgetDetail::model()->find([
                'condition' => 'parent_id = :parent',
                'params' => [':parent' => $model->budgetcomp_id],
            ]);
            ...
            aPorder::model()->updateByPk((int)$model->id, ['approved_date' => $_date]);
        }
    }
