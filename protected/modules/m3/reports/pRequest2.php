...
class pRequest2 extends fpdf
{
    ...
    function Footer()
    {
        ...
        $this->Ln();
        $this->Cell(0, 4, 'ERP Indonesia', 0, 0, 'C');
    }
    ...
    function myheader4($id)
    {
        ...
        $this->Cell(0, 6, Yii::app()->params["title"]);
        $this->Ln(3);
        ...
        $this->Cell(0, 6, peterFunc::indoFormat(aBudget::model()->findByPk($model->parent_id)->sum_af));
        $this->Ln(10);

        $w = [8, 25, 50, 30, 30, 30, 40, 50];
    ...
    function pRequestR2($id)
    {
        ...
        $this->SetFont('Arial', 'B', 8);
        $this->Cell($w[5], 6, peterFunc::indoFormat($modelt->tdebt), 'LB', 0, 'R');
        $this->Cell($w[6], 6, '', 'LB');
        $this->SetFont('Arial', '', 8);
        ...
        foreach ($models as $mod) {
            ...
            $this->Cell($w[4], 6, peterFunc::indoFormat($mod->tcredit), 'LB', 0, 'R');
            $this->Cell($w[4], 6, peterFunc::indoFormat($mod->balance), 'LB', 0, 'R');
        ...
        $this->Cell($w[4], 6, peterFunc::indoFormat($_totalc), 1, 0, 'R');
        $this->Cell($w[5], 6, peterFunc::indoFormat($mod->balance), 1, 0, 'R');
