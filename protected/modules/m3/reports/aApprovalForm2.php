...
class aApprovalForm2 extends fpdf
{
    ...
    function approvalFormR2($id)
    {
        ...
        $this->SetFont('Arial', 'B', 6);
        $this->Cell(30, 3, peterFunc::indoFormat($_amount), 0, 0, 'R');
        $this->SetFont('Arial', '', 6);
        $this->Cell(30, 3, 'Paid', 0, 0, 'R');
        $this->SetFont('Arial', 'B', 6);
        $this->Cell(20, 3, peterFunc::indoFormat($_paid), 0, 0, 'R');
        ...
        $this->SetFont('Arial', 'B', 6);
        $this->Cell(30, 3, peterFunc::indoFormat($_actual), 'B', 0, 'R');
        $this->SetFont('Arial', '', 6);
        $this->Cell(30, 3, 'UnPaid', 0, 0, 'R');
        $this->SetFont('Arial', 'B', 6);
        $this->Cell(20, 3, peterFunc::indoFormat($_unpaid), 'B', 0, 'R');
        ...
        $this->SetFont('Arial', 'B', 6);
        $this->Cell(30, 3, peterFunc::indoFormat($_currentbalance), 0, 0, 'R');
        $this->SetFont('Arial', '', 6);
        $this->Cell(30, 3, 'TOTAL', 0, 0, 'R');
        $this->SetFont('Arial', 'B', 6);
        $this->Cell(20, 3, peterFunc::indoFormat($_paid + $_unpaid), 0, 0, 'R');
        ...
        $modelp = aPorder::model()->with('po_detail')->findAll(['condition' => 'length(approved_date) <> 0 AND t.id <> ' . $id . ' AND budgetcomp_id = ' . $model->budgetcomp_id, 'order' => 't.id']);
        ...
        foreach ($modelp as $modp) {
            ...
            $this->Cell($w[2], 6, '', 'T');
            $this->Cell($w[3], 6, peterFunc::indoFormat($modp->sum_po), 'TLR', 0, 'R');
            $this->Cell($w[4], 6, '', 'T');
            $this->Cell($w[5], 6, '', 'LRT');
            $this->Ln();
            ...
            foreach ($modp->po_detail as $mod) {
                ...
                $this->Cell($w[0] + $w[1] + $w[2], 4, '     ' . $mod->budget->code . '. ' . $mod->budget->name . '. ' .
                    $_desc, 'LR');
                $this->Cell($w[3], 4, peterFunc::indoFormat($mod->qty * $mod->amount), 'LR', 0, 'R');
