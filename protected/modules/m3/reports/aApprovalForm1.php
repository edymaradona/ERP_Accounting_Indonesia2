...
class aApprovalForm1 extends fpdf
{
    ...
    function myHeader($model)
    {
        ...
        $this->Cell($w[1], 6, peterFunc::indoFormat($_amount), 'TBR', 0, 'R');
        $this->Cell($w[2], 6, peterFunc::indoFormat($_actual), 1, 0, 'R');
        $this->Cell($w[3], 6, peterFunc::indoFormat($_currentbalance), 1, 0, 'R');
        $this->Cell($w[4], 6, peterFunc::indoFormat($model->sum_po), 1, 0, 'R');
        $this->Cell($w[5], 6, peterFunc::indoFormat($_newbalance), 1, 0, 'R');
        $this->Ln();
    ...
    function approvalFormR1($id)
    {
        ...
        foreach ($modelp as $modp) {
            ...
            $this->SetFont('Arial', '', 8);
            $this->Cell($w[2], 6, peterFunc::indoFormat($modp->sum_po), 'LR', 0, 'R');
            $this->Cell($w[3], 6, '', 'LR');
            $this->Cell($w[4], 6, '', 'LR');
            $this->Cell($w[5], 6, '', 'LR');
            $this->Ln();
        ...
        foreach ($model->po_detail as $mod) {
            ...
            $this->Cell($w[2], 4, '', 'LR', 0, 'R');
            $this->Cell($w[3], 4, '', 'LR', 0, 'R');
            $this->Cell($w[4], 4, peterFunc::indoFormat($mod->qty * $mod->amount), 'LR', 0, 'R');
