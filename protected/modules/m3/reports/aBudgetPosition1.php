...
class aBudgetPosition1 extends fpdf
{
    ...
    function report($id, $pro_id)
    {
        ...
        foreach ($models as $model) {
            ...
            $this->Cell($w[2], 6, '', 'T');
            $this->Cell($w[3], 6, peterFunc::indoFormat($_amount), 'TLR', 0, 'R');
            $this->Cell($w[4], 6, '', 'T', 0, 'R');
            //$this->Cell($w[5],6,peterFunc::indoFormat($_paid,2),'T',0,'R');
            //$this->Cell($w[6],6,peterFunc::indoFormat($_unpaid,2),'T',0,'R');
            //$this->Cell($w[5],6,'','T',0,'R');
            //$this->Cell($w[6],6,'','T',0,'R');
            $this->Cell($w[7], 6, peterFunc::indoFormat($_endBalance), 'TLR', 0, 'R');
            $this->Ln();
            ...
            foreach ($model->ch as $mod) {
                ...
                $this->Cell($w[2], 4, '');
                $this->Cell($w[3], 4, peterFunc::indoFormat($mod->amount), 'LR', 0, 'R');
                ...
                $this->Cell($w[4], 4, peterFunc::indoFormat($modelT1), 0, 0, 'R');
                $_balance1 = $mod->amount - $modelT1;
                //$this->Cell($w[5],4,'',0,0,'R');
                //$this->Cell($w[6],4,'',0,0,'R');
                $this->Cell($w[7], 4, peterFunc::indoFormat($_balance1), 'LR', 0, 'R');
                $this->Ln();
                ...
                foreach ($mod->ch as $m) {
                    ...
                    $this->Cell($w[2], 4, '');
                    $this->Cell($w[3], 4, peterFunc::indoFormat($m->amount), 'LR', 0, 'R');
                    ...
                    $this->Cell($w[4], 4, peterFunc::indoFormat($modelT2), 0, 0, 'R');
                    $_balance2 = $m->amount - $modelT2;
                    //$this->Cell($w[5],4,'',0,0,'R');
                    //$this->Cell($w[6],4,'',0,0,'R');
                    $this->Cell($w[7], 4, peterFunc::indoFormat($_balance2), 'LR', 0, 'R');
                    $this->Ln();
            ...
            $this->Cell($w[2], 4, '');
            $this->Cell($w[3], 4, '', 'L');
            $this->Cell($w[4], 4, peterFunc::indoFormat($_total), 'L', 0, 'R');
            $_checkbalance = $model->amount - $_total;
            //$this->Cell($w[5],4,'',0,0,'R');
            //$this->Cell($w[6],4,'',0,0,'R');
            $this->Cell($w[7], 4, peterFunc::indoFormat($_checkbalance), 'LR', 0, 'R');
            $this->Ln();
            //*/
            $this->Cell(array_sum($w) - 46, 4, '', 'T');
            $this->Ln(1);
