...
class aPorder extends BaseModel
{
    ...
    public function rules()
    {
        ...
        return [
            ['input_date, no_ref, periode_date, budgetcomp_id', 'required'],
            ['organization_id, periode_date, budgetcomp_id, approved_date, issuer_id, payment_state_id, created_date, updated_date', 'numerical', 'integerOnly' => true],
            ['no_ref', 'length', 'max' => 100],
            ['created_by, updated_by', 'length', 'max' => 15],
            ['af_date, remark, payment_date', 'safe'],
            // The following rule is used by search().
            // Please remove those attributes that should not be searched.
            ['id, organization_id, input_date, af_date, no_ref, periode_date, budgetcomp_id, approved_date, remark, issuer_id, payment_state_id, payment_date, created_date, created_by, updated_date, updated_by', 'safe', 'on' => 'search'],
        ];
    }
    ...
    public function relations()
    {
        ...
        return [
            'sum_po' => [self::STAT, 'aPorderDetail', 'parent_id', 'select' => 'sum(qty*amount)'],
            'po_detail' => [self::HAS_MANY, 'aPorderDetail', 'parent_id'],
            'po_detail_group' => [self::HAS_MANY, 'aPorderDetail', 'parent_id', 'group' => 't.id, po_detail_group.id,po_detail_group.budget_id,po_detail_group.department_id', 'select' => '*,sum(po_detail_group.amount) as sub_total'],
            'budgetcomp' => [self::BELONGS_TO, 'aBudget', 'budgetcomp_id'],
            'organization' => [self::BELONGS_TO, 'aOrganization', 'organization_id'],
            'payment' => [self::BELONGS_TO, 'sParameter', ['payment_state_id' => 'code'], 'condition' => 'type = \'cPayment\''],
            'issuer' => [self::BELONGS_TO, 'sParameter', ['issuer_id' => 'code'], 'condition' => 'type = \'cIssuer\''],
        ];
    }
    ...
    public function getTopCreated()
    {
        ...
        $models = self::model()->with('budgetcomp')->findAll(['condition' => 'budgetcomp.parent_id = 300', 'limit' => 10, 'order' => 't.created_date DESC']);
        ...
        foreach ($models as $model) {
            ...
            $returnarray[] = ['id' => $model->no_ref, 'label' => $_no_ref, 'icon' => 'list-alt', 'url' => ['view', 'id' => $model->id]];
        }
    ...
    public function getTopUpdated()
    {
        ...
        $models = self::model()->with('budgetcomp')->findAll(['condition' => 'budgetcomp.parent_id = 300', 'limit' => 10, 'order' => 't.updated_date DESC']);
    ...
    public function getTopCreated07()
    {
        ...
        $models = self::model()->with('budgetcomp')->findAll(['condition' => 'budgetcomp.parent_id = 1001', 'limit' => 10, 'order' => 't.created_date DESC']);
    ...
    public function getTopUpdated07()
    {
        ...
        $models = self::model()->with('budgetcomp')->findAll(['condition' => 'budgetcomp.parent_id = 1001', 'limit' => 10, 'order' => 't.updated_date DESC']);
