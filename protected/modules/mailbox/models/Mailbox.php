...
class Mailbox extends CActiveRecord
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            ['initiator_id, interlocutor_id, modified', 'required'],
            ['initiator_id, interlocutor_id, bm_read, bm_deleted, initiator_del, interlocutor_del', 'numerical', 'integerOnly' => true],
            ['subject', 'length', 'max' => 100],
            ['modified', 'length', 'max' => 10],
            ['is_system', 'length', 'max' => 3],
            ['subject', 'match', 'pattern' => '/[^' . Yii::app()->controller->module->allowableCharsSubject . ']+/i', 'not' => true],
            // The following rule is used by search().
            // Please remove those attributes that should not be searched.
            ['conversation_id, initiator_id, interlocutor_id, subject, bm_read, bm_deleted, modified, is_system', 'safe', 'on' => 'search'],
            ['to', 'length', 'max' => 30],
        ];
    }
    ...
    public function relations()
    {
        ...
        return [
            'initiator' => [self::BELONGS_TO, 'User', 'initiator_id'],
            'interlocutor' => [self::BELONGS_TO, 'User', 'interlocutor_id'],
            'messages' => [self::HAS_MANY, 'Message', 'conversation_id'],
        ];
    }
    ...
    public function conversations($userid, $type = 'deleted')
    {
        ...
        $dataProvider = new CArrayDataProvider($convs, [
    ...
    {
        $unread = "";
        $unread = (self::newMsgs(Yii::app()->user->id) != 0) ? CHtml::tag("span", ['style' => 'font-size:inherit', 'class' => 'badge badge-info'], self::newMsgs(Yii::app()->user->id)) : "";
    ...
    public static function conversation($id, $order = 'DESC')
    {
        ...
        $with = ['messages' => ['order' => 'created ' . $order]];
    ...
    public function markRead($userid)
    {
        ...
        $this->updateByPk($this->conversation_id, ['bm_read' => $this->bm_read]);
    }
    ...
    public function destroy()
    {
        ...
        $msg_count = Message::model()->deleteAll('conversation_id = :cid', [':cid' => $this->conversation_id]);
        if (!$msg_count)
    ...
    {
        $available_mask_arr = [self::INITIATOR_FLAG, self::INTERLOCUTOR_FLAG];
