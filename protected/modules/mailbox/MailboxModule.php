...
class MailboxModule extends CWebModule
{
     ...
     */
    public $adminHtml = ['HTML.Allowed' => 'a[href],b,strong,p,u,i,hr,br,img[src|alt|border]'];
     ...
     * Set to an empty string to strip all HTML tags from message input.
     */
    public $html = ['HTML.Allowed' => 'a[href],b,strong,p,u,i,hr,br'];
    ...
    {
        if ($username) {
            $r = call_user_func([$this->userClass, 'model'])
                ->findByAttributes([$this->usernameColumn => $username]);
    ...
    {
        if ($userid) {
            $r = call_user_func([$this->userClass, 'model'])->findByPk($userid);
            if (!is_null($r))
        ...
        if (!$this->_username) {
            ...
            $this->_username = call_user_func([$this->userClass, 'model'])->findByPk($userid)->{$this->usernameColumn};
        }
    ...
    {
        return Yii::app()->createUrl('user', ['user' => $userid]);
    }
    ...
    public function isAdmin($userid = 0)
    {
        ...
        return call_user_func([$this->userClass, 'model'])->findByPk($userid)->{$this->superuserColumn};
    ...
    public function autoComplete($term)
    {
        ...
        $criteria->mergeWith(['limit' => 25]);
        $users = call_user_func([$this->userClass, 'model'])
            ->findAll($criteria);
    ...
    {
        $list = ['admin' => 'Site Administrator', 'support' => 'Customer Support', 'billing' => 'Billing Administrator'];
    ...
    public function registerScripts()
    {
        ...
        if ($this->cssFile !== false) {
            ...
            if ($this->cssFile === null)
                ...
                $this->cssFile = Yii::app()->request->baseUrlCdn . $this->cssFile;
            ...
            elseif ($this->menuPosition == 'left' && !$this->readOnly)
                $this->cssFileColumn = Yii::app()->request->baseUrlCdn . $this->cssFileColumn;
...
EOD;
    ...
    public static function t($str = '', $params = [], $dic = 'message')
    {
