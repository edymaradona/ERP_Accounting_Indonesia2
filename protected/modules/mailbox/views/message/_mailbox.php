        ...
        <div class="mailbox-clistview-container">
            <?php if ($dataProvider->getItemCount() > 1 && $this->getAction()->getId() != 'sent') : ?>
                <div class="btn-group mailbox-checkall-buttons">
                    <button class="btn btn-xs btn-default">Check All</button>
                    <button class="btn btn-xs btn-default">Uncheck All</button>
                </div>
            <?php
            endif;
                ...
                <div style="clear:left"><span class="btn-minis-label">With selected:</span>
                    <?php if ($this->getAction()->getId() == 'trash') : ?>
                        <input type="submit" id="mailbox-action-restore" class="btn btn-xs btn-default"
                               name="button[restore]"
                               value="restore"/>
                        <input type="submit" class="btn btn-xs btn-default" name="button[delete]"
                    ...
                    <?php else: ?>
                        <?php if (!$this->module->readOnly || ($this->module->readOnly && !$this->module->isAdmin())): ?>
                            <input type="submit" id="mailbox-action-delete" class="btn btn-xs btn-default"
                                   ...
                                   value="delete"/>
                        <?php endif; ?>
                        <input type="submit" id="mailbox-action-read" class="btn btn-xs btn-default" name="button[read]"
                               value="read"/>
