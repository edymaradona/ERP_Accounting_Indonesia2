...
class EmailManager extends CComponent
{
     ...
     * @var array
     */
    public $templateFields = ['subject', 'heading', 'message'];
     ...
     * @return bool
     */
    public function email($to, $subject, $message, $from = null, $attachments = [], $spool = true)
    {
        // get the message
    ...
    public function spool($limit = 10)
    {
        ...
        $spools = EmailSpool::model()->findAll([
     ...
     * @return array
     */
    public function buildTemplateMessage($template, $viewParams = [], $layout = 'layout_default')
    {
        $method = 'buildTemplateMessage_' . $this->templateType;
        if (!method_exists($this, $method))
            $this->templateType = 'php';
        return call_user_func_array([$this, $method], [$template, $viewParams, $layout]);
    }
     ...
     * @return array
     */
    private function buildTemplateMessage_php($template, $viewParams = [], $layout = 'layout_default')
    {
     ...
     * @return array
     */
    private function buildTemplateMessage_db($template, $viewParams = [], $layout = 'layout_default')
    {
        // load template
        $emailTemplate = EmailTemplate::model()->findByAttributes(['name' => $template]);
        if (!$emailTemplate)
            throw new CException('missing EmailTemplate - ' . $template);

        // load layout
        $emailLayout = EmailTemplate::model()->findByAttributes(['name' => $layout]);
        if (!$emailLayout)
    ...
    private function registerSwiftMailerAutoloader()
    {
        ...
        Yii::registerAutoloader(['Swift', 'autoload']);
