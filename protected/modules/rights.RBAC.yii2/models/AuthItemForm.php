...
class AuthItemForm extends CFormModel
{
    ...
    public function rules()
    {
        return [
            ...
            ['name', 'nameIsAvailable', 'on' => 'create'],
            ['name', 'newNameIsAvailable', 'on' => 'update'],
            ['name', 'isSuperuser', 'on' => 'update'],
            ['data', 'bizRuleNotEmpty'],
            ['bizRule, data', 'safe'],
        ];
    }
    ...
    public function nameIsAvailable($attribute, $params)
    {
        ...
        if (Rights::getAuthorizer()->authManager->getAuthItem($this->name) !== null)
            $this->addError('name', Rights::t('core', 'An item with this name already exists.', [':name' => $this->name]));
    }
