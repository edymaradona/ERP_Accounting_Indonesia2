...
class RDbAuthManager extends CDbAuthManager
{
    ...
    public function getAuthItems($type = null, $userId = null, $sort = true)
    {
        ...
        if ($sort === true) {
            if ($type === null && $userId === null) {
                $sql = "SELECT name,t1.type,description,t1.rule_name,t1.data,weight
                ...
                $sql = "SELECT name,t1.type,description,t1.rule_name,t1.data,weight
				FROM {$this->itemTable} t1
				LEFT JOIN {$this->assignmentTable} t2 ON name=t2.item_name
				...
				LEFT JOIN {$this->assignmentTable} t2 ON name=t2.item_name
				LEFT JOIN {$this->rightsTable} t3 ON name=t3.itemname
				WHERE t1.type=:type AND user_id=:userid
				ORDER BY t1.type DESC, weight ASC";
                $command = $this->db->createCommand($sql);
                $command->bindValue(':type', $type);
                $command->bindValue(':userid', $userId);
            ...
            foreach ($command->queryAll() as $row)
                $items[$row['name']] = new CAuthItem($this, $row['name'], $row['type'], $row['description'], $row['rule_name'], unserialize($row['data']));
        } // No sorting required.
        else {
    ...
    public function getItemChildren($names, $allowCaching = true)
    {
        ...
        if ($allowCaching && isset($this->_itemChildren[$key]) === true) {
            ...
            else if ($names === (array)$names && $names !== []) {
            ...
            $sql = "SELECT name, type, description, rule_name, data
            ...
            foreach ($this->db->createCommand($sql)->queryAll() as $row) {
                ...
                $children[$row['name']] = new CAuthItem($this, $row['name'], $row['type'], $row['description'], $row['rule_name'], $data);
            }
    ...
    {
        $sql = "SELECT * FROM {$this->assignmentTable} WHERE item_name=:itemname";
        $command = $this->db->createCommand($sql);
        ...
        foreach ($command->queryAll($sql) as $row) {
            ...
            $assignments[$row['user_id']] = new CAuthAssignment($this, $row['item_name'], $row['user_id'], $row['rule_name'], $data);
        }
        ...
        foreach ($result as $weight => $itemname) {
            ...
            if ($command->queryScalar() > 0) {
                ...
                if (($item = $this->getAuthItem($itemname)) !== null) {
                    $sql = "INSERT INTO {$this->rightsTable} (item_name, type, weight)
