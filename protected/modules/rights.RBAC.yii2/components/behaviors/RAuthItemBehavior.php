...
class RAuthItemBehavior extends CBehavior
{
    ...
    {
        return CHtml::link($this->getNameText(), [
            'authItem/update',
    ...
    {
        $markup = CHtml::link($this->owner->name, [
            'authItem/update',
            'name' => urlencode($this->owner->name),
        ]);
    ...
    {
        return CHtml::linkButton(Rights::t('core', 'Delete'), [
            'submit' => ['authItem/delete', 'name' => urlencode($this->owner->name)],
    ...
    {
        return CHtml::linkButton(Rights::t('core', 'Remove'), [
            'submit' => ['authItem/removeChild', 'name' => urlencode($this->owner->name), 'child' => urlencode($this->parent->name)],
            'class' => 'remove-link',
            'csrf' => Yii::app()->request->enableCsrfValidation,
        ]);
    }
    ...
    {
        return CHtml::linkButton(Rights::t('core', 'Remove'), [
            'submit' => ['authItem/removeChild', 'name' => urlencode($this->parent->name), 'child' => urlencode($this->owner->name)],
            'class' => 'remove-link',
            'csrf' => Yii::app()->request->enableCsrfValidation,
        ]);
    }
    ...
    {
        return CHtml::linkButton(Rights::t('core', 'Revoke'), [
            'submit' => ['assignment/revoke', 'id' => $this->userId, 'name' => urlencode($this->owner->name)],
            'class' => 'revoke-link',
            'csrf' => Yii::app()->request->enableCsrfValidation,
        ]);
    }
    ...
    public function getRevokePermissionLink(CAuthItem $role)
    {
        ...
        return CHtml::link(Rights::t('core', 'Revoke'), '#', [
            'onclick' => "
				jQuery.ajax({
				type:'POST',
				url:'" . Yii::app()->controller->createUrl('authItem/revoke', [
    ...
    public function getAssignPermissionLink(CAuthItem $role)
    {
        ...
        return CHtml::link(Rights::t('core', 'Assign'), '#', [
            'onclick' => "
				jQuery.ajax({
				type:'POST',
				url:'" . Yii::app()->controller->createUrl('authItem/assign', [
                    'name' => urlencode($role->name),
                    'child' => urlencode($this->owner->name),
                ]) . "',
				data:{ ajax:1 $csrf },
