...
{
    public function log($cmd, $ok, $context, $err = '', $errorData = []);
}
...
class elFinder
{
     ...
     * @var string
     * */
    public $_loggedCommands = ['mkdir', 'mkfile', 'rename', 'upload', 'paste', 'rm', 'duplicate', 'edit', 'resize'];
    /**
    ...
    public function __construct($options = [])
    {
        ...
        if ($this->_options['tmbDir']) {
            if (!in_array($this->_options['imgLib'], ['imagick', 'mogrify', 'gd'])) {
    ...
    public function run()
    {
        ...
        if (!empty($this->_options['logger']) && in_array($cmd, $this->_loggedCommands)) {
            $this->_options['logger']->log($cmd, empty($this->_result['error']), $this->_logContext, !empty($this->_result['error']) ? $this->_result['error'] : '', !empty($this->_result['errorData']) ? $this->_result['errorData'] : []);
        }
        exit();
    }
    /*     * ********************************************************* */
    ...
    protected function _resize()
    {
        ...
        $this->_result['select'] = [$this->_hash($target)];
    ...
    protected function _checkArchivers()
    {
        if (!function_exists('exec')) {
            $this->_options['archivers'] = $this->_options['archive'] = [];
            return;
        }
        $arcs = [
            'create' => [],
