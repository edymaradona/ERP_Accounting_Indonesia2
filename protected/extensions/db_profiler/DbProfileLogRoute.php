        ...
        foreach ($logs as $log) {
            ...
            if (!strncasecmp($message, 'begin:', 6)) {
                ...
                if (($last = array_pop($stack)) !== null && $last[0] === $token) {
                    ...
                    throw new CException(Yii::t('yii', 'CProfileLogRoute found a mismatching code block "{token}". Make sure the calls to Yii::beginProfile() and Yii::endProfile() be properly nested.', ['{token}' => $token]));
            }
        }
        $now = microtime(true);
