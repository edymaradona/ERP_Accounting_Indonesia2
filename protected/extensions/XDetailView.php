    ...
    public function run()
    {
        ...
        foreach ($this->attributes as $attribute) {
            ...
            if (isset($attribute['visible']) && !$attribute['visible'])
                continue;
            $tr = ['{label}' => '', '{class}' => $n ? $this->itemCssClass[$i % $n] : ''];
            ...
            if (isset($attribute['label']))
                ...
                if ($this->data instanceof CModel)
                    ...
                    $tr['{label}'] = ucwords(trim(strtolower(str_replace(['-', '_', '.'], ' ', preg_replace('/(?<![A-Z])[A-Z]/', ' \0', $attribute['name'])))));
            }
            ...
            if ($j == $this->ItemColumns) {
                echo CHtml::openTag($this->tagNameTR, ['class' => $n ? $this->itemCssClass[intval($i / $this->ItemColumns) % $n] : '']);
