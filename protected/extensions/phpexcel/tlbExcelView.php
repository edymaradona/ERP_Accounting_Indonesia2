...
class tlbExcelView extends TbGridView
{
        ...
        if ($this->grid_mode == 'export') {
            ...
            self::$footerStyle = [
                'borders' => [
                    'allborders' => [
                        'style' => $this->border_style,
                        'color' => ['rgb' => $this->footerBorderColor],
                    ],
                ],
                'fill' => [
                    'type' => self::$fill_solid,
                    'color' => ['rgb' => $this->footerBgColor],
                ],
                'font' => [
                    'bold' => true,
                    'color' => ['rgb' => $this->footerTextColor],
                ]
            ];
        } else {
            parent::init();
    ...
    public function renderHeader()
    {
        ...
        foreach ($this->columns as $column) {
            ...
            if (is_callable($this->onRenderHeaderCell)) {
                call_user_func_array($this->onRenderHeaderCell, [$cell, $head]);
            }
        }
    ...
    public function renderRow($row)
    {
        ...
        foreach ($this->columns as $n => $column) {
            ...
            if (is_callable($this->onRenderDataCell)) {
                call_user_func_array($this->onRenderDataCell, [$cell, $data[$row], $value]);
            }
        }
    ...
    public function renderExportButtons()
    {
        ...
        if ($content) {
            echo CHtml::tag('div', ['class' => $this->exportButtonsCSS], $this->exportText . implode(', ', $content));
        }
    }
