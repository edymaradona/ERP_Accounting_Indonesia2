...
class DynamicTabularForm extends CActiveForm
{
     ...
     */
    public function updateTypeField($model, $key, $attribute, $htmlOptions = [])
    {
        if ($model->isNewRecord)
            $model->{$attribute} = self::UPDATE_TYPE_CREATE;
        else
            $model->{$attribute} = self::UPDATE_TYPE_UPDATE;
        $htmlOptions = array_merge($htmlOptions, ['id' => get_class($model) . '_upateType_' . $htmlOptions['key']]);
     ...
     * @param array $htmlOptions
     */
    public function rowForm($models = [], $rowView = null, $htmlOptions = [])
    {
        if ($rowView == null)
            $rowView = $this->defaultRowView;
        $htmlOptions = array_merge(['id' => 'row-' . $this->rowViewCounter], $htmlOptions);
        $id = $htmlOptions['id'];
        echo CHtml::openTag('div', $htmlOptions);
        foreach ($models as $key => $model) {
            $this->controller->renderPartial($rowView, ['key' => $key, 'model' => $model, 'form' => $this]);
        }
        echo "</div>";
    ...
    public function deleteRowButton($row_id, $key, $label = 'X', $htmlOptions = [])
    {
        if (array_key_exists('class', $htmlOptions))
            $htmlOptions['class'] = $htmlOptions['class'] . ' ' . 'delete-row-button';
        else
            $htmlOptions = array_merge($htmlOptions, ['class' => 'delete-row-button']);
        $htmlOptions = array_merge($htmlOptions, ['data-delete' => $row_id, 'data-key' => $key]);
