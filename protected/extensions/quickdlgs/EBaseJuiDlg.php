...
abstract class EBaseJuiDlg extends CWidget
{
    ...
    {
        $juiOptions = isset($this->dialogAttributes['options']) ? $this->dialogAttributes['options'] : [];
    ...
    public function renderButton($config)
    {
        ...
        $buttonHtmlOptions = isset($config['htmlOptions']) ? $config['htmlOptions'] : [];
    ...
    public function renderDialogOpenButton()
    {
        ...
        $buttonHtmlOptions = array_merge($this->openButtonHtmlOptions, ['onclick' => $onClick]);
        $config = [
