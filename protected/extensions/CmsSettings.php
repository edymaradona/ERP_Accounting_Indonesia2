...
{
    protected $_saveItemsToDatabase = [];
    protected $_deleteItemsFromDatabase = [];
    protected $_deleteCategoriesFromDatabase = [];
    protected $_cacheNeedsFlush = [];
    protected $_items = [];
    protected $_loaded = [];
    ...
    {
        parent::init();
        Yii::app()->attachEventHandler('onEndRequest', [$this, 'whenRequestEnds']);
        ...
        if (is_array($key)) {
            ...
            if ($toDatabase) {
                if (isset($this->_saveItemsToDatabase[$category]) && is_array($this->_saveItemsToDatabase[$category]))
                    $this->_saveItemsToDatabase[$category] = array_merge($this->_saveItemsToDatabase[$category], [$key => $value]);
                else
                    $this->_saveItemsToDatabase[$category] = [$key => $value];
            }
            if (isset($this->_items[$category]) && is_array($this->_items[$category]))
                $this->_items[$category] = array_merge($this->_items[$category], [$key => $value]);
            else
        ...
        if (is_array($key)) {
            ...
            if (isset($this->_items[$category][$key])) {
                ...
                if (empty($this->_deleteItemsFromDatabase[$category]))
                    $this->_deleteItemsFromDatabase[$category] = [];
