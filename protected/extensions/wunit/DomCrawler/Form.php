...
class Form extends Link implements \ArrayAccess
{
    ...
    public function getUri()
    {
        ...
        if (!in_array($this->getMethod(), ['POST', 'PUT', 'DELETE']) && $queryString = http_build_query($this->getValues(), null, '&')) {
    ...
    private function initialize()
    {
        ...
        foreach ($xpath->query('descendant::input | descendant::textarea | descendant::select', $root) as $node) {
            ...
            } elseif ('input' == $nodeName && !in_array($node->getAttribute('type'), ['submit', 'button', 'image'])) {
                $this->set(new Field\InputFormField($node));
    ...
    {
        $this->button = $node;
        if ('button' == $node->nodeName || ('input' == $node->nodeName && in_array($node->getAttribute('type'), ['submit', 'button', 'image']))) {
            do {
